...
<list-files> | <check-russian> | <convert-tables> | <filter-images> | <format-code> | <help> |
<repair-links> | <reduce-images> | <repair-svg> | <substitute> | <set-table-cols> | <get-terms> |
<validate-yaml>
...
Подкоманды:
  ...
  reduce-images                          Команда для уменьшения веса изображений JPG,
                                         PNG
  repair-links                           Команда для проверки и исправления ссылок в
                                         файлах документации
  repair-svg                             Команда для исправления файлов SVG
  set-table-cols                         Команда для задания ширины столбцам таблиц
  substitute                             Команда для замены переменных на их
                                         значения
  validate-yaml / check                  Команда для валидации YAML-файла,
                                         используемого при генерации PDF
----
