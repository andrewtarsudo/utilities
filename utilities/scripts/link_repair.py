...
from utilities.common.errors import (
    LinkRepairInvalidFileDictAttributeError,
    LinkRepairInvalidStorageAttributeError,
)
...
def has_no_required_files(path: StrPath) -> bool:
    bool_md: bool = (
        next(iglob(f"**/*{MD_EXTENSION}", root_dir=path, recursive=True), None) is None
    )
    bool_adoc: bool = (
        next(iglob(f"**/*{ADOC_EXTENSION}", root_dir=path, recursive=True), None)
        is None
    )
...
def link_repair_command(
    ...
    if anchor_validation:
        ...
        for file, anchors in anchor_inspector.dict_changes.items():
            for anchor in anchors:
                ...
                addition: str = (
                    file.full_path.stem.lower()
                    .removeprefix("_")
                    .replace("_", "-")
                    .replace(".", "-")
                )
                file.update_line(
                    line_number, anchor, f"{anchor}-{addition}", is_boundary=True
                )
    ...
    for dir_file in iter(file_dict):
        ...
        dir_file._content = file_reader(
            dir_file.full_path, ReaderMode.LINES, encoding="utf-8"
        )
        ...
        for link_item in dir_file.iter_links():
            ...
            if not bool(link_inspector):
                _file_path: str = relpath(
                    link_inspector.link.from_file, _base_dir
                ).replace("\\", "/")
                logger.error(
    ...
    if dry_run:
        logger.info(
            "==================== Файлы не изменены ====================", result=True
        )

    else:
        logger.info(
            "==================== Файлы изменены ====================", result=True
        )
