...
class File:
    ...
    def __init__(self, path: StrPath):
        ...
        self._content: list[str] = file_reader(
            self._path, ReaderMode.LINES, encoding="utf-8"
        )
    ...
    def imagesdir(self):
        for line in iter(self):
            if line.startswith("ifndef::imagesdir") and ":imagesdir:" in (
                ...
                imagesdir: str = (
                    line_shorten.removeprefix("[:imagesdir:").strip().removesuffix("]")
                )
...
def filter_images_command(
    ...
    md_paths: list[StrPath] | None = get_files(
        ctx, directory=project_dir, recursive=True, language=None, extensions="md"
    )
    md_files: list[MdFile] = [MdFile(md_path) for md_path in md_paths]

    adoc_paths: list[StrPath] | None = get_files(
        ctx, directory=project_dir, recursive=True, language=None, extensions="adoc"
    )

    adoc_files: list[AsciiDocFile] = [
        AsciiDocFile(adoc_path) for adoc_path in adoc_paths
    ]

    links_paths_names: dict[Path, str] = {
        Path(full_link).resolve(): full_link.name
        for file in [*md_files, *adoc_files]
        for full_link in file.full_links
    }
    ...
    if images_paths_names:
        _: list[Path] = [
            unused_image.relative_to(project_dir) for unused_image in images_paths_names
        ]
        ...
        if output is not None:
            bool_convert: dict[bool, str] = {True: "Да", False: "Нет"}

            info: str = (
                f"Директория {project_dir}, рекурсивно: {bool_convert.get(recursive)}\n"
            )
