...
class TableCoordinate(NamedTuple):
    """Class to represent the cell coordinates in the table.
    ...
    def as_element(cls, number: int, num_columns: int):
        """Generates the coordinate from the table index.

        :param number: The table cell index.
        :type number: int
    ...
    def shift(self, row_offset: int = 0, column_offset: int = 0):
        """Generates the table coordinates displaced from the given one.
        ...
        :return: The new instance with the specified place in the table grid.
        """
