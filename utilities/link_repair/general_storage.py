...
class GeneralStorage:
    ...
    def __eq__(self, other):
        ...
        elif issubclass(self.__class__, other.__class__) or issubclass(
    ...
    def _set_dir_indexes(self, paths: Iterable[Path]):
        ...
        _unique_adoc: list[str] = unique(
            iter(
                self.parent_name(_)
                for _ in iglob(
                    "**/_index.adoc", root_dir=self._root_dir, recursive=True
                )
            )
        )
    ...
    def _set_text_files(self, paths: Iterable[Path]):
        ...
        for _path in paths:
            ...
            if _name in _unique:
                ...
                _name_grandparent: str = (
                    f"{_path.parent.parent.name}/{_path.parent.name}/{_path.stem}"
                )
...
class Storage(GeneralStorage):
    ...
    def is_empty(self) -> bool:
        ...
        if not self._components_path.exists():
            ...
            bool_md: bool = self._components_path.joinpath(
                f"_index{MD_EXTENSION}"
            ).exists()
            bool_asciidoc: bool = self._components_path.joinpath(
                f"_index{ADOC_EXTENSION}"
            ).exists()
    ...
    def _component_storage_names(self):
        if self.is_empty:
            ...
            return [
                item.name for item in self._components_path.iterdir() if item.is_dir()
            ]

    def _set_component_storages(self):
        """Specifies ComponentStorage items."""
        for _name in self._component_storage_names:
            component_storage: ComponentStorage = ComponentStorage(
                self._root_dir, _name
            )
