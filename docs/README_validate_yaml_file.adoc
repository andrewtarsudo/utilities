= Руководство для validate_yaml_file.py
:experimental:
:icons: font
...
:script-name: validate_yaml_file
...
----
$ py <path/to/{script-name}.py> <OPTIONS> // <1> <2>
----

<1> path/to/{script-name}.py -- путь до скрипта `{script-name}.py`
<2> OPTIONS -- опции скрипта

=== Опции скрипта

NOTE: Здесь предполагается, что скрипт находится в директории, откуда запускается, +
т.е. `<path/to/{script-name}.py>` -> `{script-name}.py`.

[source,console,subs=+attributes]
----
Использование:
py {script-name}.py [ -h/--help | -v/--version ] YAML_FILE [ -o/--output OUTPUT ] [ --verbose ]
...
. Загрузить скрипт по https://gitlab.com/tech_writers_protei/scripts/-/raw/main/scripts/{script-name}.py?inline=false[ссылке].
...
. В консоли указать команду `py ..{backslash}{script-name}.py`.

При желании записи в файл команда изменится на `py ..{backslash}{script-name}.py -o "<file_name>.log"` +
или `py ..{backslash}{script-name}.py --output="<file_name>.log"`.
